...
class Zombie:
    ...
    def get_bb(self):
        # Frame width가 100일 때와 아닐 때 각각 다른 값을 반환하도록 수정
        if self.frame_width == 100:
            left, bottom, right, top = self.x - 25, self.y - 40, self.x + 25, self.y + 30
        else:
            left, bottom, right, top = self.x - 50, self.y - 70, self.x + 50, self.y + 70
